...
# Create 1000 VCFs with 50% misgenotyped samples
...
# Assign different samples to the real VCF for 50%
# Re-run Fisher
...
# 50% samples are assigned the other social type
# misgenotyping model with Fisher's exact test and Bonferroni adjustment
todayanalysis="2021-01-13-misgenotyping-50-Fisher-Bonferroni${ticker}"

# Step 1: Change the social type of 50% of samples to the VCF file
...
# number that I should change: 54 (= 50%)

# sample randomly that number
shuf -n 54 result/sample_names.txt > tmp/${todayanalysis}_sample_names_to_misgenotype.txt
