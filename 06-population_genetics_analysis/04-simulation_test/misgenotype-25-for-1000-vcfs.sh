...
# Create 1000 VCFs with 25% misgenotyped samples
...
# Assign different samples to the real VCF for 25%
# Re-run Fisher
...
# 25% samples are assigned the other social type
# misgenotyping model with Fisher's exact test and Bonferroni adjustment
todayanalysis="2021-01-13-misgenotyping-25-Fisher-Bonferroni${ticker}"

# Step 1: Change the social type of 25% of samples to the VCF file
...
# number that I should change: 27 (= 25%)

# sample randomly that number
shuf -n 27 result/sample_names.txt > tmp/${todayanalysis}_sample_names_to_misgenotype.txt
