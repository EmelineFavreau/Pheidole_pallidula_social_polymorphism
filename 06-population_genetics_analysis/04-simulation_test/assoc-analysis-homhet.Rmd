---
title: "Association analysis Pheidole + fake SNP homhet"
...
54,738 biallelic SNPs, in linkage equilibrium, supported by at least 75% of samples, on coding-only regions (loss of 80% of all SNPs by remooving non-coding contigs). 
One of the SNPs is fake: monogynous samples are homozygote for alt, polygynous are heterozygote.
...
plink_output <- read.csv("result/2019-04-02-flye-Pheidole-genic-fakesnp-homhet-LDpruned-maf0.05-snp-pvalues-nocovar.assoc.logistic.filtered", header = FALSE, sep = "")
# head(plink_output)
...
scaff_length_df <- rbind(c("contig_01_pilon_pilon_pilon_pilon_pilon_pilon_pilon_pilon_pilon_pilon", "72141"),
      scaff_length_df)
...
manhattan(manhattan_df_ten_ANDfake_longuest_contigs,
          ...
          main = "Regression analysis: SNPs in linkage equilibrium in all populations \n for 10 longest scaffolds and fake green SNP homhet", xlab = "Scaffolds ordered by length")
...
plink_output <- read.csv("result/2019-04-02-flye-Pheidole-genic-fakesnp-homhet.assoc.fisher", header = TRUE, sep = "")
# head(plink_output)
...
- Fisher analysis does isolate my fake SNP (hom-het).
